...
export const Route = createFileRoute("/onboarding")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tell us a little about yourself so Project Netr can surface opportunities designed for you." },
    ],
  }),
  ...
  return (
    <div className="min-h-screen netr-hero-bg">
      <header className="mx-auto flex max-w-5xl items-center justify-between px-4 py-6 sm:px-6">
        ...
        <span className="text-xs text-muted-foreground">{t("onboarding.stepCounter", { step: step + 1, total: TOTAL_STEPS })}</span>
      </header>
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
                  ...
                  {personas.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-background/80"><Icon className="h-5 w-5" /></span>
                        <h3 className="mt-4 font-semibold">{t(`personas.${p.id}.title`)}</h3>
                        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t(`personas.${p.id}.description`)}</p>
                        {active && <Check className="absolute right-3 top-3 h-4 w-4 text-saffron" />}
                      </button>
                    );
                  })}
                </div>
              ...
              <StepLayout title={t("onboarding.s2.title")} hint={t("onboarding.s2.hint")}>
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="netr-card p-5">
                    <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("onboarding.s2.stateLabel")}</span>
                    <select value={data.state ?? ""} onChange={(e) => set("state", e.target.value)} className="mt-2 w-full bg-transparent text-base font-medium outline-none">
                      <option value="">{t("onboarding.s2.selectState")}</option>
                      {states.map((s) => <option key={s} className="bg-background">{s}</option>)}
                    </select>
                  </label>
                  <label className="netr-card p-5">
                    <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{t("onboarding.s2.districtLabel")}</span>
                    <input value={data.district ?? ""} onChange={(e) => set("district", e.target.value)} placeholder={t("onboarding.s2.districtPlaceholder")} className="mt-2 w-full bg-transparent text-base font-medium outline-none placeholder:text-muted-foreground/60" />
                  </label>
                </div>
              </StepLayout>
            )}

            {step === 2 && <ChoiceStep title={t("onboarding.s3.title")} hint={t("onboarding.s3.hint")} options={ageGroups} value={data.age} onChange={(v) => set("age", v)} />}
            {step === 3 && <ChoiceStep title={t("onboarding.s4.title")} hint={t("onboarding.s4.hint")} options={education} value={data.education} onChange={(v) => set("education", v)} />}
            {step === 4 && <ChoiceStep title={t("onboarding.s5.title")} hint={t("onboarding.s5.hint")} options={income} value={data.income} onChange={(v) => set("income", v)} />}
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border/60 bg-background/85 backdrop-blur-xl">
          <div className="mx-auto flex max-w-3xl items-center justify-between gap-3 px-4 py-4 sm:px-6">
            <button onClick={back} className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm text-muted-foreground hover:text-foreground">
              <ArrowLeft className="h-4 w-4" /> {t("onboarding.back")}
            </button>
            <button onClick={next} className="inline-flex items-center gap-2 rounded-full bg-foreground px-6 py-3 text-sm font-semibold text-background transition-transform hover:scale-[1.03]">
...
function StepLayout({ title, hint, children }: { title: string; hint?: string; children: React.ReactNode }) {
  return (
    <div>
...
function ChoiceStep({ title, hint, options, value, onChange }: { title: string; hint?: string; options: string[]; value?: string; onChange: (v: string) => void }) {
  return (
