...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A quiet, personal discovery feed of opportunities surfaced just for you.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
  ...
  const recQuery = useQuery({
    ...
    queryFn: () =>
      hasProfile
        ? findEligiblePrograms(profile, 6)
        : listAllOpportunities().then((p) => p.slice(0, 6)),
  });
    ...
    <AppShell>
      ...
      >
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          {!hasProfile && (
            <Link
              to="/onboarding"
              className="inline-flex items-center gap-2 rounded-full bg-saffron px-4 py-2 text-xs font-semibold text-primary-foreground"
            >
          ...
          <Link
            to="/opportunities"
            className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/60 px-4 py-2 text-xs font-semibold"
          >
      ...
      <section className="mt-12">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <QuickAction
            icon={GraduationCap}
            label={t("dashboard.qa.scholarships.label")}
            sub={t("dashboard.qa.scholarships.sub")}
            onClick={() => navigate({ to: "/scholarships" })}
          />
          <QuickAction
            icon={Landmark}
            label={t("dashboard.qa.schemes.label")}
            sub={t("dashboard.qa.schemes.sub")}
            onClick={() => navigate({ to: "/schemes" })}
          />
          <QuickAction
            icon={Sparkles}
            label={t("dashboard.qa.ai.label")}
            sub={t("dashboard.qa.ai.sub")}
            onClick={() => navigate({ to: "/ai-assistant" })}
          />
          <QuickAction
            icon={Upload}
            label={t("dashboard.qa.documents.label")}
            sub={t("dashboard.qa.documents.sub")}
      ...
      <section className="mt-14">
        <SectionHead
          title={hasProfile ? t("dashboard.recommendedTitle") : t("dashboard.featuredTitle")}
          link="/opportunities"
          linkLabel={t("dashboard.seeAll")}
        />
        {recQuery.isLoading ? (
          <div className="netr-card flex items-center justify-center p-10 text-sm text-muted-foreground">
            <Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t("dashboard.loadingOpps")}
          </div>
        ) : (
      ...
      <section className="mt-14">
        <SectionHead
          title={`${t("dashboard.bookmarksTitle")}${bookmarkedQuery.data?.length ? ` (${bookmarkedQuery.data.length})` : ""}`}
        />
        {bookmarkedQuery.data && bookmarkedQuery.data.length > 0 ? (
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {bookmarkedQuery.data.map((o, i) => (
              <OpportunityCard key={o.id} opportunity={o} index={i} />
            ))}
          </div>
        ) : (
      ...
      <section className="mt-14">
        <SectionHead
          title={t("dashboard.askAnythingTitle")}
          link="/ai-assistant"
          linkLabel={t("dashboard.openAssistant")}
        />
...
function QuickAction({
  ...
  return (
    <button
      onClick={onClick}
      className="netr-card netr-card-hover group flex items-center gap-3 p-4 text-left"
    >
