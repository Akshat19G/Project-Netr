...
export function OpportunityCard({ opportunity, index = 0 }: { opportunity: Program; index?: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm leading-relaxed text-muted-foreground/90">{opportunity.description}</p>

      <div className="mt-5 grid grid-cols-2 gap-3 text-xs">
        <div className="rounded-xl border border-border/60 px-3 py-2">
          <div className="flex items-center gap-1.5 text-muted-foreground"><Coins className="h-3 w-3" /> {t("card.benefitLabel")}</div>
          <div className="mt-0.5 font-semibold">{opportunity.benefits}</div>
        </div>
        <div className="rounded-xl border border-border/60 px-3 py-2">
          <div className="flex items-center gap-1.5 text-muted-foreground"><Clock className="h-3 w-3" /> {t("card.windowLabel")}</div>
      ...
      {expanded && (
        <div className="mt-4 space-y-3 text-xs">
          <div>
            <p className="font-semibold uppercase tracking-wider text-muted-foreground">{t("card.eligibility")}</p>
            <ul className="mt-1 list-disc space-y-0.5 pl-4 text-foreground/80">
              {opportunity.eligibility.map((e) => <li key={e}>{e}</li>)}
            </ul>
          </div>
          <div>
            <p className="font-semibold uppercase tracking-wider text-muted-foreground">{t("card.benefits")}</p>
            <ul className="mt-1 list-disc space-y-0.5 pl-4 text-foreground/80">
              {opportunity.benefitsDetail.map((e) => <li key={e}>{e}</li>)}
            </ul>
          </div>
        </div>
      )}
      ...
      <p className="mt-2 text-[10px] uppercase tracking-wider text-muted-foreground/70">{t("card.source")} · {opportunity.source}</p>
      </div>
    </motion.article>
  );
}
